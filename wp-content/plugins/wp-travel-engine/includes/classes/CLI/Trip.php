...
class Trip extends WP_CLI_Command {
	...
	protected function insert_package( $trip_id ) {
		...
		foreach ( $terms as $term_id => $term_name ) {
			...
			$package_categories['enabled_sale'][ $term_id ]  = (bool) array_rand( array( 0, 1 ) );
		...
		update_post_meta( $trip_id, 'packages_ids', array( $package_id ) );
	...
	public function generate( $args, $assoc_args ) {
		...
		$trip_duration_unit = $faker->randomElement( array( 'days', 'hours' ) );
			...
			array(
				...
				'meta_input'   => array(
					...
					'wp_travel_engine_setting'      => array(
						...
						'trip_facts'             => array(
							'field_id'   => array( 'Accommodation', 'Admission Fee', 'Language', 'Guides' ),
							'field_type' => array( 'text', 'text', 'text', 'text' ),
							0            => array( 0 => '5 Star Hotel' ),
							1            => array( 1 => 'No' ),
							2            => array( 2 => 'English, Deutsch' ),
							3            => array( 3 => 'Yes' ),
						),
					),
				),
		...
		$activities     = array( 'Tour', 'Trekking', 'Day Tour', 'Adventure', 'Hiking', 'Short Tour' );
	...
	public function reset() {
		$trips = new WP_Query(
			array(
				'post_type'      => array( 'trip', 'booking', 'enquiry', 'customer', 'wte-coupon' ),
				'posts_per_page' => - 1,
			)
		);
