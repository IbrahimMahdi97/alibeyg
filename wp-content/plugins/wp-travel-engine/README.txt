...
= 6.6.9 - 27th September 2025 =

* Improved: Enabled translation support for Trip Data using WPML Advanced Translation Editor.
* Improved: Side filters in the Trip Search page are now sorted alphabetically for easier navigation.
* Improved: SEO Analysis now supports content created with the default Block Editor in the Trip Edit page.
* Changed: Removed the decimal digit setting from the core plugin.
* Changed: Amounts now display exactly as entered, while checkout calculations are rounded to two decimal places.
* Fixed: Resolved an issue where the selected featured image for a trip was not displaying correctly on the frontend, ensuring it now consistently appears as intended across all trip views.
