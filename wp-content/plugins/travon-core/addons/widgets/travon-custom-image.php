 ...
 *
 */
class Travon_Image extends Widget_Base {
	...
	public function get_title() {
		return __( 'Travon Image Box', 'travon' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'image_wrapper_class',
			[
				'label'     => __( 'Image Wrapper Class', 'travon' ),
        ...
        $this->add_control(
			'show_vdo',
			[
				'label' 		=> __( 'Show Video Button?', 'travon' ),
		...
		$this->add_control(
			'video_btn_color',
			[
				'label' 	=> __( 'Video Button Color', 'travon' ),
		...
		$this->add_control(
			...
			[
				'label' 	=> __( 'Video Button Hover Color', 'travon' ),
				'type' 		=> Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_control(
			...
			[
				'label' 	=> __( 'Video Button Background Color', 'travon' ),
				'type' 		=> Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_control(
			...
			[
				'label' 	=> __( 'Video Button Background Hover Color', 'travon' ),
				'type' 		=> Controls_Manager::COLOR,
				'selectors' => [
		...
		$this->add_control(
			...
			[
				'label' 		=> __( 'Video Button Ripple Effect Color', 'travon' ),
				'type' 			=> Controls_Manager::COLOR,
				'selectors' 	=> [
	...
	protected function render() {
        ...
        if($settings['show_vdo'] == 'yes'){
	        $this->add_render_attribute('wrapper','class', 'ot-video' );
	        $this->add_render_attribute('wrapper','data-overlay', 'title' );
	        $this->add_render_attribute('wrapper','data-opacity', '5' );
	    }
        ...
        if( !empty( $settings['image']['id'] ) ) {
            echo '<!-- Image -->';
                echo '<div '.$this->get_render_attribute_string('wrapper').'>';
					...
					echo '<img class="'.esc_attr( $settings['image_class'] ).'" src="'.esc_url( Group_Control_Image_Size::get_attachment_image_src($settings['image']['id'],'image',$settings) ).'" alt="'.esc_attr( travon_image_alt( Group_Control_Image_Size::get_attachment_image_src($settings['image']['id'],'image',$settings) ) ).'" >';
					...
					if($settings['show_vdo'] == 'yes'){
						if( !empty( $settings['video_link']['url'] ) ) {
							echo '<a href="'.esc_url($settings['video_link']['url']).'" class="play-btn style3 popup-video"><i class="fas fa-play"></i></a>';
						}						
					}
                echo '</div>';
            echo '<!-- End Image -->';
