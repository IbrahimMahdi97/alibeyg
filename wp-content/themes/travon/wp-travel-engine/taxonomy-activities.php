            ...
            <?php
                ...
                if($termchildren) {
                    ...
                    if($wte_trip_tax_post_qry->have_posts()) : ?>
                        <div class="category-main-wrap category-grid row gy-30   grid <?php echo esc_attr( $termID ); ?>" data-id="<?php echo esc_attr( $wte_trip_tax_post_qry->max_num_pages );?>">
                            <?php 
                    ...
                    foreach ($termchildren as $child) {
                        ...
                        ?>
                        <div class="category-main-wrap category-grid row gy-30  grid <?php echo esc_attr( $child_term->term_id ); ?>" data-id="<?php echo esc_attr( $my_query->max_num_pages ); ?>">
                            <?php 
                                ...
                                if( $child_term->count > $default_posts_per_page ){
                                    echo "<div class='btn-loadmore' data-query-vars='".json_encode( $my_query->query_vars )."' data-current-page='".esc_attr( $paged )."' data-max-page='".esc_attr( $my_query->max_num_pages )."'><span>".__("Load More Trips","travel-booking")."</span></div>";
                                }
                            ?>
                        </div>
                        <?php
                    ...
                    if($wte_trip_tax_post_qry->have_posts()) : ?>
                        <div class="category-main-wrap category-grid row gy-30  grid <?php echo esc_attr( $termID );?>" data-id="<?php echo esc_attr( $wte_trip_tax_post_qry->max_num_pages ); ?>">
                            <?php
                                ...
                                wp_reset_query();
                                if( $term_act->count > $default_posts_per_page ){
                                    echo "<div class='btn-loadmore' data-query-vars='".json_encode( $wte_trip_tax_post_qry->query_vars )."' data-current-page='".esc_attr( $paged )."' data-max-page='".esc_attr( $wte_trip_tax_post_qry->max_num_pages )."'><span>".__("Load More Trippps","travel-booking")."</span></div>";
                                }
                            ?>
                        </div>
                        <?php
