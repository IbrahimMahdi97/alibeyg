...
/*	F	*/
function mk_jdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){

    $T_sec=0;/* <= رفع خطاي زمان سرور ، با اعداد '+' و '-' بر حسب ثانيه */

    if($time_zone!='local')date_default_timezone_set(($time_zone==='')?'Asia/Tehran':$time_zone);
    $ts=$T_sec+(($timestamp==='')?time():mk_tr_num($timestamp));
    ...
    for($i=0; $i<$sl; $i++){
        ...
        switch($sub){
            ...
            case'k';
                $out.=mk_tr_num(100-(int)($doy/($kab+365)*1000)/10,$tr_num);
                break;

            case'K':
                $out.=mk_tr_num((int)($doy/($kab+365)*1000)/10,$tr_num);
                break;
    ...
    return($tr_num!='en')?mk_tr_num($out,'fa','.'):$out;
}

/*	F	*/
function mk_jstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
...
/*	F	*/
function mk_jmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$none='',$timezone='Asia/Tehran'){
    if($timezone!='local')date_default_timezone_set($timezone);
    if($h===''){
        return time();
    }else{
        list($h,$m,$s,$jm,$jd,$jy)=explode('_',mk_tr_num($h.'_'.$m.'_'.$s.'_'.$jm.'_'.$jd.'_'.$jy));
        if($m===''){
            return mktime($h);
        }else{
            if($s===''){
...
/*	F	*/
function mk_jgetdate($timestamp='',$none='',$timezone='Asia/Tehran',$tn='en'){
    $ts=($timestamp==='')?time():mk_tr_num($timestamp);
    $jdate=explode('_',jdate('F_G_i_j_l_n_s_w_Y_z',$ts,'',$timezone,$tn));
    return array(
        'seconds'=>mk_tr_num((int)mk_tr_num($jdate[6]),$tn),
        'minutes'=>mk_tr_num((int)mk_tr_num($jdate[2]),$tn),
        'hours'=>$jdate[1],
        'mday'=>$jdate[3],
        'wday'=>$jdate[7],
        'mon'=>$jdate[5],
...
/*	F	*/
function mk_jdate_words($array,$mod=''){
    foreach($array as $type=>$num){
        $num=(int)mk_tr_num($num);
        switch($type){
...
/*	F	*/
function mk_gregorian_to_jalali($gy,$gm,$gd,$mod=''){
    list($gy,$gm,$gd)=explode('_',mk_tr_num($gy.'_'.$gm.'_'.$gd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
...
/*	F	*/
function mk_jalali_to_gregorian($jy,$jm,$jd,$mod=''){
    list($jy,$jm,$jd)=explode('_',mk_tr_num($jy.'_'.$jm.'_'.$jd));/* <= Extra :اين سطر ، جزء تابع اصلي نيست */
    if($jy > 979){
        $gy=1600;
        $jy-=979;
    }else{
